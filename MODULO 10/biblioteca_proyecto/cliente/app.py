...
NOTIFY_EMAIL = os.getenv("NOTIFY_EMAIL", "").split(",")
...
def editar(libro_id):
    try:
        r = requests.get(f"{API_URL}/books/{libro_id}")
        r.raise_for_status()
        libro = r.json()
    except Exception:
    ...
    if request.method == "POST":
        ...
        try:
            r = requests.put(f"{API_URL}/books/{libro_id}", json=data)
...
def eliminar(libro_id):
    try:
        r = requests.delete(f"{API_URL}/books/{libro_id}")
        if r.status_code == 200:
...
def buscar():
    ...
    if filtro:
        try:
            ...
            for libro in r.json():
                if (
                    filtro in libro.get("titulo","").lower() or
                    filtro in libro.get("autor","").lower() or
                    filtro in libro.get("genero","").lower()
                ):
